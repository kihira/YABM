...
import kihira.yabm.inventory.slot.SlotBackpack;
...
public class ContainerBackpack extends Container {
    ...
    public ContainerBackpack(IInventory invPlayer, IInventory invBackpack) {
        ...
        for (j = 0; j < 3; ++j) {
            for (k = 0; k < 9; ++k) {
                if (invPlayer.getStackInSlot(k + j * 9 + 9) != null && invPlayer.getStackInSlot(k + j * 9 + 9).getItem() instanceof IBackpack) this.addSlotToContainer(new SlotLocked(invPlayer, k + j * 9 + 9, 8 + k * 18, 103 + j * 18 + i));
                else this.addSlotToContainer(new Slot(invPlayer, k + j * 9 + 9, 8 + k * 18, 103 + j * 18 + i));
            }
        }

        for (j = 0; j < 9; ++j) {
            if (invPlayer.getStackInSlot(j) != null && invPlayer.getStackInSlot(j).getItem() instanceof IBackpack) this.addSlotToContainer(new SlotLocked(invPlayer, j, 8 + j * 18, 161 + i));
